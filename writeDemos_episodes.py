...
#                 self.replay_buffer.add(*buf)
    ...
    def single_demo_pick_place(self, iteration, cfg, record = False):
        ...
        while self.step < cfg.episodeLength:
            ...
            # switchboard
            # print(np.linalg.norm(raw_dict["gripper_force"]))
            # print(np.linalg.norm(raw_dict["object_sound"]))
...
#         return success
    ...
    def run(self, cfg):
        counter = 0
        successes = 0
        func_dict = {"Lift": self.single_demo_lift, "MMLift": self.single_demo_lift,
                    "IndicatorBoxBlock": self.single_demo_indicator_box, "BlockedPickPlace": self.single_demo_pick_place}
